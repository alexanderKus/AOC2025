...
function expandInstructions(start, ins) {
    let pos = start;
    const positions = [pos];
...
function drawPointer(index) {
    ...
    // Update info text
    if (infoEl) {
        infoEl.textContent = `Current position: ${index}   ·   Step ${currentStepIndex} / ${positions.length - 1}`;
    }
}
...
function animate(timestamp) {
    ...
    if (stepsToAdvance > 0) {
        ...
        currentStepIndex = Math.min(currentStepIndex + stepsToAdvance, positions.length - 1);
    }
    drawClockFace();
